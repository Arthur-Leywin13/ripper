...
const PROFILE_PIC_PATH = path.join(__dirname, '..', 'assets', 'img', 'profile.jpg')
const profilePicDone = new Set() // évite de renvoyer la photo de profil à chaque reconnexion
 ...
 * @param {{sock, jid}} [opts.notifyTarget] - une session déjà connectée à qui envoyer le QR/code
 * @param {(qr: string) => void} [opts.onQr] - callback recevant le QR brut (ex: pour une page web)
 * @param {(phoneNumber: string) => void} [opts.onOpen] - callback appelé à la connexion, avec le numéro
 */
async function startSession(sessionId, opts = {}) {
  ...
  sessions.set(sessionId, { sock, status: 'connecting', phoneNumber: null })
  ...
  if (pairNumber && !sock.authState.creds.registered) {
    try {
      ...
      if (notifyTarget) await notifyTarget.sock.sendMessage(notifyTarget.jid, { text })
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log(`✅ [${sessionId}] ${config.BOT_NAME} connecté (${phoneNumber}).`)
      if (notifyTarget) {
        await notifyTarget.sock.sendMessage(notifyTarget.jid, { text: `Session *${sessionId}* connectée.` })
      }
      if (!profilePicDone.has(sessionId) && fs.existsSync(PROFILE_PIC_PATH)) {
        profilePicDone.add(sessionId)
        try {
          await sock.updateProfilePicture(sock.user.id, fs.readFileSync(PROFILE_PIC_PATH))
        } catch (e) {
          console.error(`[${sessionId}] Impossible de définir la photo de profil:`, e.message)
        }
      }
      if (onOpen) onOpen(phoneNumber)
    }
  })
